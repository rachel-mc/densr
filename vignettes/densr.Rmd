...
Typically, these functions are applied sequentially (used in the order they are listed above). See below for a typical workflow of the `densr` package.
...
The 'type' argument consists of a character string denoting the valid options for different types of density estimation. `match.arg()` and the intelligible error messages included in the function additionally check that the user correctly supplied _one_ of the following.   If no type is specified, the default is used and otherwise execution is terminated:  
...
legend("topleft", legend = c("rectangular", "biweight"), lty = c(1,3))
```
...
```{r}
par(mfrow = c(1,2))
plot(scott, main = "", border = "darkorchid2", xlab = "Linolenic acid content in olives")
plot(seq, main = "", border = "dodgerblue2", xlab = "Linolenic acid content in olives")
mtext("Density histograms with Scott's breaks (purple) and Sequence breaks (blue)", side = 1, line = -21, outer = TRUE)
```
